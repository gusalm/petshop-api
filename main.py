...
def listar_produtos():
    nome = request.args.get('product_name')
    preco = request.args.get('product_price')
    estoque = request.args.get('stock_quantity')

    resultado = products

    if nome:
        resultado = [p for p in resultado if nome.lower() in p['product_name'].lower()]

    if preco:
        preco = float(preco)
        resultado = [p for p in resultado if p['product_price'] <= preco]

    if estoque:
        estoque = int(estoque)
        resultado = [p for p in resultado if p['stock_quantity'] >= estoque]
